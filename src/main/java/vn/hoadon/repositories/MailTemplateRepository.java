...
public interface MailTemplateRepository extends JpaRepository<MailTemplateEntity, Integer> {
    ...
    /** Look up company-specific templates (system = 1) by key and companyId. */
    @Query("SELECT t FROM MailTemplateEntity t WHERE t.key = :key AND t.company.id = :companyId AND t.system = 1 ORDER BY t.status DESC, t.id ASC")
    ...
    /** Look up a system-wide template (system = 0) by key, ignoring company. */
    @Query("SELECT t FROM MailTemplateEntity t WHERE t.key = :key AND t.system = 0 ORDER BY t.id ASC")
